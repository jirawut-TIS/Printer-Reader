...
export default function LoginPage({ onLogin }) {
  ...
  return (
    <div style={{
      ...
      background: "#0d1117", fontFamily: "'Sarabun',sans-serif",
    }}>
      <div style={{
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div>
            ...
            <input type="text" value={username} onChange={e => setUsername(e.target.value)}
              placeholder="กรอกชื่อผู้ใช้" required autoFocus style={inp} />
          </div>
          <div>
            <label style={{ fontSize: 12, color: "#8b949e", display: "block", marginBottom: 5 }}>รหัสผ่าน</label>
            <input type="password" value={password} onChange={e => setPassword(e.target.value)}
              placeholder="กรอกรหัสผ่าน" required style={inp} />
          </div>

          {error && (
            <div style={{
              padding: "10px 14px", borderRadius: 8, fontSize: 13,
              background: "rgba(248,81,73,.1)", border: "1px solid #f85149", color: "#fca5a5",
            }}>⚠️ {error}</div>
          )}

          <button type="submit" disabled={loading} style={{
            padding: "12px 0", border: "none", borderRadius: 9, fontSize: 15, fontWeight: 700,
            cursor: loading ? "not-allowed" : "pointer", marginTop: 4,
            background: loading ? "#1e293b" : "linear-gradient(135deg,#38bdf8,#6366f1)",
            color: loading ? "#4b5563" : "#0d1117",
        ...
        <div style={{
          marginTop: 20, padding: "12px 14px", background: "#0d1117",
          borderRadius: 8, fontSize: 12, color: "#4b5563", lineHeight: 2,
          border: "1px solid #21262d",
        }}>
          <span style={{ color: "#8b949e", fontWeight: 600 }}>บัญชีทดสอบ</span><br />
          admin / admin123<br />
          printer1 / op1234<br />
          printer2 / op5678
        </div>
