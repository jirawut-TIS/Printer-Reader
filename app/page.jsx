...
export default function Home() {
  ...
  function buildRows(collected) {
    ...
    // Re-sort values by page (Map preserves insertion order which is sorted)
    return [...map.values()];
  }
  ...
  const handleRead = async () => {
    ...
    try {
      ...
      const collected  = [];   // { page, serial, model, printA4, printA5, grandTotal }
      const nullPages  = [];   // หน้าที่ Claude บอกว่า null (ไม่มีข้อมูล)
      let   processed  = 0;
      ...
      /* ── Retry null pages that might be actual Usage pages ── */
      // (null pages > 15% of total → something went wrong, retry)
      const realNullCount = nullPages.length;
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.header}>
        ...
        <div>
          <div style={S.hTitle}>Printer Usage Reader v3.1</div>
          <div style={S.hSub}>HP LaserJet · English + Thai · เรียงตามหน้า PDF · {WORKERS} Workers</div>
        </div>
      </div>
      ...
      <div style={S.wrap}>
        ...
        <div style={{marginTop:32,textAlign:"center",fontSize:11,color:"#30363d"}}>
          Printer Usage Reader v3.1 · sort by PDF page · English + Thai · retry on failure
        </div>
      </div>
    </div>
  );
